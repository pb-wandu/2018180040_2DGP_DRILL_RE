...
class Boy:
    ...
    def fire_ball(self):
        ...
        if self.ball_count > 0:
            ...
            # boy가 발사하는 ball
            ball_boy = Ball(self.x, self.y, self.face_dir*10)
            game_world.add_object(ball_boy)

            # 충돌 처리 대상 조합의 a 위치에 ball을 추가
            game_world.add_collision_pair("ball-zombie", ball_boy, None)
    ...
    def handle_collision(self, group, other):
        ...
        # zombie와 소년의 ball이 충돌한 경우
        if group == "zombie-ball_boy":
            # 소년의 공을 월드에서 제거한다
            game_world.remove_object(self.ball_boy)
